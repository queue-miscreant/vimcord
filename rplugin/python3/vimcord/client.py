...
class VimcordClient(discord.Client):
    def __init__(self, bridge):
        self.bridge = bridge
        self.plugin = bridge.plugin
        super().__init__(loop=self.plugin.nvim.loop)
        ...
        async def on_init():
            log.info("Initting discord")
            await self.login()
            await self.connect()
        self.plugin.nvim.loop.create_task(on_init())
    ...
    async def get_new_servers(self):
        ...
        self._getting_servers = False
        self.bridge.on_discord_connected()
    ...
    #-----MESSAGE EVENTS--------------------------------------------------------
    async def on_message(self, post: discord.Message):
        '''Push new message'''
        if not isinstance(post.channel, discord.channel.PrivateChannel) \
        and self.is_muted(post.server, post.channel):
            return
        try:
            self.bridge.on_message(post)
    ...
    async def on_message_edit(self, before, after):
        ...
        try:
            self.bridge.on_message_edit(after)
        except Exception as e:
            log.error(e, stack_info=True)

    async def on_message_delete(self, post):
        '''Remove message if we receive a deletion'''
        try:
            self.bridge.on_message_delete(post)
        except Exception as e:
            log.error(e, stack_info=True)

    async def on_dm_update(self, dm):
        '''Show presence on right hand side'''
        try:
            self.bridge.on_dm_update(dm)
        except Exception as e:
            log.error(e, stack_info=True)

    #EXTERNAL METHODS-----------------------------------------------------------
    async def prepend_log(self, limit=5):
        '''Prepend messages from server'''
        async for item in self.logs_from(self._channel, limit=limit):
            # prepend messages to buffer?
            pass
